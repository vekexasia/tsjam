...
use bandersnatch::{IetfProof, Output, Input, RingProof, Public};
...
#[napi]
pub fn vrf_verify(signature: &[u8], vrf_input_data: &[u8], aux_data: &[u8], commitment: Buffer, ring_size: u32 ) -> bool {
    ...
    ! Public::verify(
        input, output, aux_data, &signature.proof, &verifier
    ).is_err()
}
