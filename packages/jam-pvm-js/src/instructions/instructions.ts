...
import { PVMExitReasonImpl, toSafeMemoryAddress } from "@tsjam/pvm-base";
import { PVMRegisterRawValue, u32, u8 } from "@tsjam/types";
import { type PVMJS } from "..";
...
const _1Buf = Buffer.alloc(1);
const _2_Buf = Buffer.alloc(2);
const _4_Buf = Buffer.alloc(4);
const _8_Buf = Buffer.alloc(8);
/**
...
export class Instructions {
  ...
  @BlockTermination
  fallthrough(_: NoArgIxArgs, pvm: PVMJS) {
    pvm.pc = <u32>(pvm.pc + 1);
  }
  ...
  store_imm_u8({ vX, vY }: TwoImmArgs, pvm: PVMJS) {
    return storeSafe(vX, Buffer.from([Number(vY % 256n)]), pvm);
  }

  @Ix(31, TwoImmIxDecoder)
  store_imm_u16({ vX, vY }: TwoImmArgs, pvm: PVMJS) {
    _2_Buf.writeUint16LE(Number(vY % 2n ** 16n));
    return storeSafe(vX, _2_Buf, pvm);
  }

  @Ix(32, TwoImmIxDecoder)
  store_imm_u32({ vX, vY }: TwoImmArgs, pvm: PVMJS) {
    _4_Buf.writeUint32LE(Number(vY % 2n ** 32n));
    return storeSafe(vX, _4_Buf, pvm);
  }

  @Ix(33, TwoImmIxDecoder)
  store_imm_u64({ vX, vY }: TwoImmArgs, pvm: PVMJS) {
    _8_Buf.writeBigUint64LE(vY);
    return storeSafe(vX, _8_Buf, pvm);
  }

  @Ix(40, OneOffsetIxDecoder, true)
  @BlockTermination
  jump({ ipOffset }: OneOffsetArgs, pvm: PVMJS, skip: number) {
    return branch(pvm, ipOffset, true, skip);
  }

  @Ix(50, OneRegOneImmIxDecoder, true)
  @BlockTermination
  jump_ind({ wA, vX }: OneRegOneImmArgs, pvm: PVMJS) {
    const jumpLocation = Number((wA.value + vX) % 2n ** 32n) as u32;
    return djump(jumpLocation, pvm);
  }
  ...
  load_u8({ wA, vX }: OneRegOneImmArgs, pvm: PVMJS) {
    const memoryAddress = toSafeMemoryAddress(vX);

    if (!pvm.memory.canRead(memoryAddress, 1)) {
      return handleMemoryFault(pvm.memory.firstUnreadable(memoryAddress, 1)!);
    }
    pvm.memory.readInto(memoryAddress, _1Buf);
    wA.value = <PVMRegisterRawValue>BigInt(_1Buf[0]);
  }

  @Ix(54, OneRegOneImmIxDecoder)
  load_u16({ wA, vX }: OneRegOneImmArgs, pvm: PVMJS) {
    const memoryAddress = toSafeMemoryAddress(vX);

    if (!pvm.memory.canRead(memoryAddress, 2)) {
      return handleMemoryFault(pvm.memory.firstUnreadable(memoryAddress, 2)!);
    }
    pvm.memory.readInto(memoryAddress, _2_Buf);
    const memVal = _2_Buf.readUInt16LE();
    wA.value = <PVMRegisterRawValue>BigInt(memVal);
  }

  @Ix(56, OneRegOneImmIxDecoder)
  load_u32({ wA, vX }: OneRegOneImmArgs, pvm: PVMJS) {
    const memoryAddress = toSafeMemoryAddress(vX);
    if (!pvm.memory.canRead(memoryAddress, 4)) {
      return handleMemoryFault(pvm.memory.firstUnreadable(memoryAddress, 4)!);
    }

    pvm.memory.readInto(memoryAddress, _4_Buf);
    const memVal = _4_Buf.readUInt32LE();

    wA.value = <PVMRegisterRawValue>BigInt(memVal);
  }

  @Ix(58, OneRegOneImmIxDecoder)
  load_u64({ wA, vX }: OneRegOneImmArgs, pvm: PVMJS) {
    const memoryAddress = toSafeMemoryAddress(vX);
    if (!pvm.memory.canRead(memoryAddress, 8)) {
      return handleMemoryFault(pvm.memory.firstUnreadable(memoryAddress, 8)!);
    }

    pvm.memory.readInto(memoryAddress, _8_Buf);

    wA.value = <
      PVMRegisterRawValue // E_8
    >_8_Buf.readBigUint64LE();
  }

  // ### Load signed
  @Ix(53, OneRegOneImmIxDecoder)
  load_i8({ wA, vX }: OneRegOneImmArgs, pvm: PVMJS) {
    const memoryAddress = toSafeMemoryAddress(vX);
    if (!pvm.memory.canRead(memoryAddress, 1)) {
      return handleMemoryFault(pvm.memory.firstUnreadable(memoryAddress, 1)!);
    }
    ...
    pvm.memory.readInto(memoryAddress, _1Buf);
    wA.value = <PVMRegisterRawValue>X_1(BigInt(_1Buf[0]));
  }

  @Ix(55, OneRegOneImmIxDecoder)
  load_i16({ wA, vX }: OneRegOneImmArgs, pvm: PVMJS) {
    const memoryAddress = toSafeMemoryAddress(vX);
    if (!pvm.memory.canRead(memoryAddress, 2)) {
      return handleMemoryFault(pvm.memory.firstUnreadable(memoryAddress, 2)!);
    }
  ...
  load_i32({ wA, vX }: OneRegOneImmArgs, pvm: PVMJS) {
    const memoryAddress = toSafeMemoryAddress(vX);
    if (!pvm.memory.canRead(memoryAddress, 4)) {
      return handleMemoryFault(pvm.memory.firstUnreadable(memoryAddress, 4)!);
    }
  ...
  store_u8({ wA, vX }: OneRegOneImmArgs, pvm: PVMJS) {
    return storeSafe(
  ...
  store_u16({ wA, vX }: OneRegOneImmArgs, pvm: PVMJS) {
    _2_Buf.writeUint16LE(Number(wA.value % 2n ** 16n));
    return storeSafe(toSafeMemoryAddress(vX), _2_Buf, pvm);
  }

  @Ix(61, OneRegOneImmIxDecoder)
  store_u32({ wA, vX }: OneRegOneImmArgs, pvm: PVMJS) {
    _4_Buf.writeUint32LE(Number(wA.value % 2n ** 32n));
    return storeSafe(toSafeMemoryAddress(vX), _4_Buf, pvm);
  }

  @Ix(62, OneRegOneImmIxDecoder)
  store_u64({ wA, vX }: OneRegOneImmArgs, pvm: PVMJS) {
    _8_Buf.writeBigUint64LE(wA.value);
    return storeSafe(toSafeMemoryAddress(vX), _8_Buf, pvm);
  }

  @Ix(70, OneRegTwoImmIxDecoder)
  store_imm_ind_u8({ wA, vX, vY }: OneRegTwoImmArgs, pvm: PVMJS) {
  ...
  store_imm_ind_u16({ wA, vX, vY }: OneRegTwoImmArgs, pvm: PVMJS) {
    const location = wA.value + vX;
    _2_Buf.writeUint16LE(Number(vY % 2n ** 16n));
    return storeSafe(toSafeMemoryAddress(location), _2_Buf, pvm);
  }

  @Ix(72, OneRegTwoImmIxDecoder)
  store_imm_ind_u32({ wA, vX, vY }: OneRegTwoImmArgs, pvm: PVMJS) {
    const location = wA.value + vX;
    _4_Buf.writeUint32LE(Number(vY % 2n ** 32n));
    return storeSafe(toSafeMemoryAddress(location), _4_Buf, pvm);
  }

  @Ix(73, OneRegTwoImmIxDecoder)
  store_imm_ind_u64({ wA, vX, vY }: OneRegTwoImmArgs, pvm: PVMJS) {
    const location = wA.value + vX;
    _8_Buf.writeBigUint64LE(vY);

    return storeSafe(toSafeMemoryAddress(location), _8_Buf, pvm);
  }
  ...
  branch_eq_imm(
    ...
    return branch(pvm, ipOffset, wA.value === vX, skip);
  }
  ...
  branch_lt_s_imm(
    ...
    return branch(pvm, ipOffset, Z8(wA.value) < Z8(vX), skip);
  }
  ...
  @BlockTermination
  load_imm_jump_ind(args: TwoRegTwoImmIxArgs, pvm: PVMJS) {
    const r = djump(Number((args.wB.value + args.vY) % 2n ** 32n) as u32, pvm);
  ...
  branch_eq(
    ...
    return branch(pvm, ipOffset, wA.value === wB.value, skip);
  }
  ...
  branch_lt_s(
    ...
    return branch(pvm, ipOffset, Z8(wA.value) < Z8(wB.value), skip);
  }
  ...
  branch_ge_u(
    ...
    return branch(pvm, ipOffset, wA >= wB, skip);
  }
  ...
  store_ind_u8({ wB, vX, wA }: TwoRegOneImmArgs, pvm: PVMJS) {
  ...
  store_ind_u16({ wA, wB, vX }: TwoRegOneImmArgs, pvm: PVMJS) {
    const location = toSafeMemoryAddress(wB.value + vX);
    _2_Buf.writeUint16LE(Number(wA.value & 0xffffn));
    return storeSafe(location, _2_Buf, pvm);
  }

  @Ix(122, TwoRegOneImmIxDecoder)
  store_ind_u32({ wA, wB, vX }: TwoRegOneImmArgs, pvm: PVMJS) {
    const location = toSafeMemoryAddress(wB.value + vX);
    _4_Buf.writeUInt32LE(Number(wA.value % 2n ** 32n));
    return storeSafe(location, _4_Buf, pvm);
  }

  @Ix(123, TwoRegOneImmIxDecoder)
  store_ind_u64({ wA, wB, vX }: TwoRegOneImmArgs, pvm: PVMJS) {
    const location = toSafeMemoryAddress(wB.value + vX);
    _8_Buf.writeBigUint64LE(wA.value);
    return storeSafe(location, _8_Buf, pvm);
  }

  // # load unsigned
  @Ix(124, TwoRegOneImmIxDecoder)
  load_ind_u8({ wA, wB, vX }: TwoRegOneImmArgs, pvm: PVMJS) {
    const location = toSafeMemoryAddress(wB.value + vX);
    if (!pvm.memory.canRead(location, 1)) {
      return handleMemoryFault(pvm.memory.firstUnreadable(location, 1)!);
    }
    pvm.memory.readInto(location, _1Buf);
    wA.value = <PVMRegisterRawValue>BigInt(_1Buf[0]);
  }

  @Ix(126, TwoRegOneImmIxDecoder)
  load_ind_u16({ wA, wB, vX }: TwoRegOneImmArgs, pvm: PVMJS) {
    const location = toSafeMemoryAddress(wB.value + vX);
    if (!pvm.memory.canRead(location, 2)) {
      return handleMemoryFault(pvm.memory.firstUnreadable(location, 2)!);
    }
    pvm.memory.readInto(location, _2_Buf);
    wA.value = <PVMRegisterRawValue>BigInt(_2_Buf.readUint16LE());
  }

  @Ix(128, TwoRegOneImmIxDecoder)
  load_ind_u32({ wA, wB, vX }: TwoRegOneImmArgs, pvm: PVMJS) {
    const location = toSafeMemoryAddress(wB.value + vX);
    if (!pvm.memory.canRead(location, 4)) {
      return handleMemoryFault(pvm.memory.firstUnreadable(location, 4)!);
    }
    pvm.memory.readInto(location, _4_Buf);
    wA.value = <PVMRegisterRawValue>BigInt(_4_Buf.readUInt32LE());
  }

  @Ix(130, TwoRegOneImmIxDecoder)
  load_ind_u64({ wA, wB, vX }: TwoRegOneImmArgs, pvm: PVMJS) {
    const location = toSafeMemoryAddress(wB.value + vX);
    if (!pvm.memory.canRead(location, 8)) {
      return handleMemoryFault(pvm.memory.firstUnreadable(location, 8)!);
    }
    pvm.memory.readInto(location, _8_Buf);

    wA.value = <PVMRegisterRawValue>_8_Buf.readBigUInt64LE(0);
  }

  // # load signed
  @Ix(125, TwoRegOneImmIxDecoder)
  load_ind_i8({ wA, wB, vX }: TwoRegOneImmArgs, pvm: PVMJS) {
    const location = toSafeMemoryAddress(wB.value + vX);
    if (!pvm.memory.canRead(location, 1)) {
      return handleMemoryFault(pvm.memory.firstUnreadable(location, 1)!);
    }
    pvm.memory.readInto(location, _1Buf);
    const val = Z8_inv(Z1(BigInt(_1Buf[0])));
    wA.value = <PVMRegisterRawValue>val;
  }

  @Ix(127, TwoRegOneImmIxDecoder)
  load_ind_i16({ wA, wB, vX }: TwoRegOneImmArgs, pvm: PVMJS) {
    const location = toSafeMemoryAddress(wB.value + vX);
    if (!pvm.memory.canRead(location, 2)) {
      return handleMemoryFault(pvm.memory.firstUnreadable(location, 2)!);
    }
    pvm.memory.readInto(location, _2_Buf);
    const num = _2_Buf.readUint16LE();
    wA.value = <PVMRegisterRawValue>Z8_inv(Z2(BigInt(num)));
  }

  @Ix(129, TwoRegOneImmIxDecoder)
  load_ind_i32({ wA, wB, vX }: TwoRegOneImmArgs, pvm: PVMJS) {
    const location = toSafeMemoryAddress(wB.value + vX);
    if (!pvm.memory.canRead(location, 4)) {
      return handleMemoryFault(pvm.memory.firstUnreadable(location, 4)!);
    }
    pvm.memory.readInto(location, _4_Buf);
    const num = _4_Buf.readUInt32LE();
...
const storeSafe = (location: u32, bytes: Buffer, pvm: PVMJS) => {
