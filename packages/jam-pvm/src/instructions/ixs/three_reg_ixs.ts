...
const add = create(8 as u8, "add", (context, wA, wB, rD) => {
  context.execution.registers[rD] = ((wA + wB) % 2 ** 32) as u32;
});

const sub = create(20 as u8, "sub", (context, wA, wB, rD) => {
  context.execution.registers[rD] = ((wA + 2 ** 32 - wB) % 2 ** 32) as u32;
});

const and = create(23 as u8, "and", (context, wA, wB, rD) => {
  context.execution.registers[rD] = (wA & wB) as u32;
});
...
const mul_upper_s_s = create(
  ...
  "mul_upper_s_s",
  (context, wA, wB, rD) => {
    context.execution.registers[rD] = Z4_inv(
      Math.floor((Z4(wA) * Z4(wB)) / 2 ** 32),
    );
  },
);
...
const mul_upper_u_u = create(
  ...
  "mul_upper_u_u",
  (context, wA, wB, rD) => {
    context.execution.registers[rD] = Math.floor((wA * wB) / 2 ** 32) as u32;
  },
);
...
const div_u = create(68 as u8, "div", (context, wA, wB, rD) => {
  if (wB === 0) {
    context.execution.registers[rD] = (2 ** 32 - 1) as u32;
  } else {
    context.execution.registers[rD] = Math.floor(wA / wB) as u32;
  }
});
...
const div_s = create(64 as u8, "div_s", (context, wA, wB, rD) => {
  ...
  if (wB === 0) {
    ...
    context.execution.registers[rD] = wA;
  } else {
    context.execution.registers[rD] = Z4_inv(Math.floor(z4a / z4b));
  }
});
...
const rem_s = create(70 as u8, "rem_s", (context, wA, wB, rD) => {
  ...
  if (wB === 0) {
    ...
    context.execution.registers[rD] = Z4_inv(z4a % z4b);
  }
});

const set_lt_u = create(36 as u8, "set_lt_u", (context, wA, wB, rD) => {
  context.execution.registers[rD] = (wA < wB ? 1 : 0) as u32;
});

const set_lt_s = create(58 as u8, "set_lt_s", (context, wA, wB, rD) => {
  const z4a = Z4(wA);
  const z4b = Z4(wB);
  context.execution.registers[rD] = (z4a < z4b ? 1 : 0) as u32;
});

const shlo_l = create(55 as u8, "shlo_l", (context, wA, wB, rD) => {
  context.execution.registers[rD] = ((wA << wB % 32) % 2 ** 32) as u32;
});
...
const shar_r = create(77 as u8, "shar_r", (context, wA, wB, rD) => {
  const z4a = Z4(wA);
  context.execution.registers[rD] = Z4_inv(Math.floor(z4a / 2 ** (wB % 32)));
});
  ...
  describe("three_reg_ixs", () => {
    ...
    describe("ixs", () => {
      ...
      it("add => rD = (rA + rB)%2^32", () => {
        ...
        expect(context.execution.registers[rD]).toBe(3);

        context.execution.registers[rA] = (2 ** 32 - 1) as u32;
        add.evaluate(context, rA, rB, rD);
        expect(context.execution.registers[rD]).toBe(1);
      });
      it("sub => rD = (rA + 2^32 - rB)%2^32", () => {
        context.execution.registers[rA] = 4 as u32;
        context.execution.registers[rB] = 2 as u32;
        sub.evaluate(context, rA, rB, rD);
        expect(context.execution.registers[rD]).toBe(2);

        context.execution.registers[rA] = 1 as u32;
        sub.evaluate(context, rA, rB, rD);
        expect(context.execution.registers[3]).toBe(2 ** 32 - 1);
      });
      it("and => rD = rA & rB", () => {
        context.execution.registers[rA] = 0b1010 as u32;
        context.execution.registers[rB] = 0b1100 as u32;
        and.evaluate(context, rA, rB, rD);
        expect(context.execution.registers[rD]).toBe(0b1000);
      });
      it("xor => rD = rA ^ rB", () => {
        context.execution.registers[rA] = 0b1010 as u32;
        context.execution.registers[rB] = 0b1100 as u32;
        xor.evaluate(context, rA, rB, rD);
        expect(context.execution.registers[rD]).toBe(0b0110);
      });
      it("or => rD = rA | rB", () => {
        context.execution.registers[rA] = 0b1010 as u32;
        context.execution.registers[rB] = 0b1100 as u32;
        or.evaluate(context, rA, rB, rD);
        expect(context.execution.registers[rD]).toBe(0b1110);
      });
      it("mul => rD = (rA * rB)%2^32", () => {
        ...
        expect(context.execution.registers[rD]).toBe(200);

        context.execution.registers[rA] = (2 ** 31) as u32;
        context.execution.registers[rB] = 2 as u32;
        mul.evaluate(context, rA, rB, rD);
        expect(context.execution.registers[rD]).toBe(0);
      });
      it("mul_upper_s_s => rD = Z4_inv(Z4(rA) * Z(4, rB) / 2^32)", () => {
        ...
        for (let i = 2; i < 30; i++) {
          ...
          expect(context.execution.registers[rD]).toBe(2 ** (i - 2));
        }
        // - * +
        ...
        for (let i = 2; i < 30; i++) {
          context.execution.registers[rB] = (2 ** i) as u32;
          mul_upper_s_s.evaluate(context, rA, rB, rD);
          expect(Z4(context.execution.registers[rD])).toBe(-1 * 2 ** (i - 2));
        }
        ...
        for (let i = 2; i < 30; i++) {
          ...
          mul_upper_s_s.evaluate(context, rA, rB, rD);
          expect(Z4(context.execution.registers[rD])).toBe(2 ** (i - 2));
        }
      });
      it("mul_upper_u_u => rD = (rA * rB) / 2^32", () => {
        ...
        expect(context.execution.registers[rD]).toBe(10);
      });
      ...
      it("div_u => rD = rA / rB", () => {
        ...
        // edgecase div by 0
        expect(context.execution.registers[rD]).toBe(2 ** 32 - 1);
      ...
      it("div_s => rD = Z4_inv(Z4(rA) / Z4(rB))", () => {
        ...
        div_s.evaluate(context, rA, rB, rD);
        expect(context.execution.registers[rD]).toBe(Z4_inv(-20));
        ...
        div_s.evaluate(context, rA, rB, rD);
        expect(context.execution.registers[rD]).toBe(
          context.execution.registers[rA],
        );
      });
      it("rem_u => rD = rA % rB", () => {
      ...
      it("rem_s => rD = Z4_inv(Z4(rA) % Z4(rB))", () => {
        ...
        expect(context.execution.registers[rD]).toBe(Z4_inv(2));
      });
      ...
      it("shlo_l => rD = rA << rB % 32", () => {
        ...
        expect(context.execution.registers[rD]).toBe(0b10100);

        context.execution.registers[rB] = 2 as u32;
        shlo_l.evaluate(context, rA, rB, rD);
        expect(context.execution.registers[rD]).toBe(0b101000);
      ...
      it("shlo_r => rD = rA >> rB % 32", () => {
        ...
        expect(context.execution.registers[rD]).toBe(0b101);

        context.execution.registers[rB] = 2 as u32;
        shlo_r.evaluate(context, rA, rB, rD);
        expect(context.execution.registers[rD]).toBe(0b10);
      ...
      it("shar_r => rD = Z4_inv(Z4(rA) / 2^(rB % 32))", () => {
        ...
        context.execution.registers[rB] = 1 as u32;
        shar_r.evaluate(context, rA, rB, rD);
        expect(context.execution.registers[rD]).toBe(Z4_inv(-1 * 2 ** 30));
      });
      ...
      it("cmov_nz => rD = rB !== 0 ? rA : rD", () => {
        ...
        context.execution.registers[rB] = 2 as u32;
        cmov_nz.evaluate(context, rA, rB, rD);
        expect(context.execution.registers[rD]).toBe(12);
      });
    });
  });
}
