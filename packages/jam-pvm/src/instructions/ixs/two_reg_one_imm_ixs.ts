...
const load_ind_i8 = create(21 as u8, "load_ind_i8", (context, rA, rB, vX) => {
  ...
  context.registers[rA] = Z4_inv(Z(1, val));
});
...
const mul_upper_s_s_imm = create(
  ...
  "mul_upper_s_s_imm",
  (context, rA, rB, vX) => {
    context.registers[rB] = Z4_inv(
      Math.floor((Z4(context.registers[rA]) * Z4(vX)) / 2 ** 32),
    );
  },
);
...
const shlo_l_imm = create(9 as u8, "shlo_l_imm", (context, rA, rB, vX) => {
  context.registers[rB] = ((context.registers[rA] << vX % 32) % 2 ** 32) as u32;
});
...
const shar_r_imm = create(25 as u8, "shar_r_imm", (context, rA, rB, vX) => {
  context.registers[rB] = Z4_inv(Z4(context.registers[rA]) >> vX % 32);
});
...
const shar_r_imm_alt = create(
  ...
  "shar_r_imm_alt",
  (context, rA, rB, vX) => {
    context.registers[rB] = Z4_inv(Z4(vX) >> context.registers[rA] % 32);
  },
);
...
const set_lt_s_imm = create(56 as u8, "set_lt_s_imm", (context, rA, rB, vX) => {
  context.registers[rB] = (Z4(context.registers[rA]) < Z(4, vX) ? 1 : 0) as u32;
});
  ...
  describe("two_reg_one_imm_ixs", () => {
    ...
    describe("ixs", () => {
      it("store_ind_u8", () => {
        ...
        context.registers[0] = 0x1020 as u32;
        context.registers[1] = 0x1010 as u32;
      ...
      it("store_ind_u32", () => {
        const context = createEvContext();
        context.registers[0] = 0x10203040 as u32;
        context.registers[1] = 0x1010 as u32;
      ...
      it("load_ind_u8", () => {
        const context = createEvContext();
        (context.memory.get as Mock).mockReturnValueOnce(0x20);
        context.registers[0] = 0x1010 as u32;
        context.registers[1] = 0 as u32;
        ...
        expect(context.registers[1]).toBe(0x20);
        expect((context.memory.get as Mock).mock.calls).toEqual([[0x1021]]);
      });
      it("load_ind_u16", () => {
        const context = createEvContext();
        ...
        expect(context.registers[1]).toBe(0x1020);
        expect((context.memory.getBytes as Mock).mock.calls).toEqual([
          [0x1021, 2],
        ]);
      });
      it("load_ind_u32", () => {
        ...
        expect(context.registers[1]).toBe(0x10203040);
        expect((context.memory.getBytes as Mock).mock.calls).toEqual([
          [0x1021, 4],
        ]);
      });
      it("load_ind_i8", () => {
        const context = createEvContext();
        (context.memory.get as Mock).mockReturnValueOnce(Z_inv(1, -1));
        context.registers[0] = 0x1010 as u32;
        context.registers[1] = 0 as u32;
        ...
        expect(context.registers[1]).toBe(Z4_inv(-1));
        expect((context.memory.get as Mock).mock.calls).toEqual([[0x1021]]);
      });
      it("load_ind_i16", () => {
        const context = createEvContext();
      ...
      it("add_imm", () => {
        const context = createEvContext();
        context.registers[0] = 0xffffffff as u32;
        add_imm.evaluate(
          context,
          0 as RegisterIdentifier,
          1 as RegisterIdentifier,
        ...
        expect(context.registers[1]).toBe(2);
      });
      it("and_imm", () => {
        const context = createEvContext();
        context.registers[0] = 0b101 as u32;
        and_imm.evaluate(
          context,
          0 as RegisterIdentifier,
          1 as RegisterIdentifier,
        ...
        expect(context.registers[1]).toBe(0b100);
      });
      it("xor_imm", () => {
        const context = createEvContext();
        context.registers[0] = 0b101 as u32;
        ...
        expect(context.registers[1]).toBe(0b011);
      });
      it("or_imm", () => {
        const context = createEvContext();
        context.registers[0] = 0b101 as u32;
        ...
        expect(context.registers[1]).toBe(0b111);
      });
      it("mul_imm", () => {
        const context = createEvContext();
        context.registers[0] = (2 ** 31) as u32;
        mul_imm.evaluate(
          context,
          0 as RegisterIdentifier,
          1 as RegisterIdentifier,
        ...
        expect(context.registers[1]).toBe(0);
        ...
        expect(context.registers[1]).toBe(2 ** 31);
      });
      it("mul_upper_s_s_imm", () => {
        const context = createEvContext();
        context.registers[0] = Z4_inv(2 ** 30);
        ...
        expect(context.registers[1]).toBe(Z4_inv(2));
        ...
        expect(context.registers[1]).toBe(Z4_inv(-4));
      });
      it("mul_upper_u_u_imm", () => {
        const context = createEvContext();
        context.registers[0] = (2 ** 30) as u32;
      ...
      it("set_lt_u_imm", () => {
        const context = createEvContext();
        context.registers[0] = 0x10 as u32;
        ...
        expect(context.registers[1]).toBe(1);
        context.registers[0] = 0x11 as u32;
        set_lt_u_imm.evaluate(
          context,
          0 as RegisterIdentifier,
          1 as RegisterIdentifier,
      ...
      it("set_gt_u_imm", () => {
        ...
        expect(context.registers[1]).toBe(0);
        context.registers[0] = 0x12 as u32;
        set_gt_u_imm.evaluate(
          context,
          0 as RegisterIdentifier,
          1 as RegisterIdentifier,
      ...
      it("cmov_iz_imm", () => {
        const context = createEvContext();
        context.registers[0] = 0 as u32;
        context.registers[1] = 1 as u32;
        ...
        expect(context.registers[0]).toBe(0x0);
        context.registers[1] = 0 as u32;
        cmov_iz_imm.evaluate(
          context,
          0 as RegisterIdentifier,
        ...
        expect(context.registers[0]).toBe(0x10);
      });
      it("cmov_nz_imm", () => {
        const context = createEvContext();
        context.registers[0] = 0 as u32;
      ...
      it("shlo_r_imm", () => {
        const context = createEvContext();
        context.registers[0] = 0x80000000 as u32;
        shlo_r_imm.evaluate(
          context,
          0 as RegisterIdentifier,
          1 as RegisterIdentifier,
        ...
        expect(context.registers[1]).toBe(0x40000000);
      });
      it("shlo_r_imm_alt", () => {
        const context = createEvContext();
        context.registers[0] = 1 as u32;
      ...
      it("shar_r_imm", () => {
        const context = createEvContext();
        context.registers[0] = Z4_inv(-1 * 2 ** 31) as u32;
        shar_r_imm.evaluate(
          context,
          0 as RegisterIdentifier,
          1 as RegisterIdentifier,
        ...
        expect(context.registers[1]).toBe(Z4_inv(-1 * 2 ** 30));
      });
      it("shar_r_imm_alt", () => {
        const context = createEvContext();
        context.registers[0] = 33 as u32;
