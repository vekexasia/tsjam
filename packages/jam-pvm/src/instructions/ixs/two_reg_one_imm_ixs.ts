...
export type TwoRegOneImmIxsArgs = ReturnType<typeof TwoRegOneImmIxsDecoder>;

const create = (
  opCode: number,
  identifier: string,
  evaluate: PVMIxEvaluateFN<TwoRegOneImmIxsArgs>,
) => {
  return regIx({
    opCode: opCode as u8,
    identifier,
    decode: TwoRegOneImmIxsDecoder,
    evaluate,
    gasCost: 1n as Gas,
  });
};
...
create(120, "store_ind_u8", ({ wB, vX, wA }) => {
  const location = toSafeMemoryAddress(wB + vX);
  return [IxMod.memory(location as u32, new Uint8Array([Number(wA & 0xffn)]))];
});

create(121 as u8, "store_ind_u16", ({ wA, wB, vX }) => {
  const location = toSafeMemoryAddress(wB + vX);
  return [IxMod.memory(location, encodeWithCodec(E_2, wA & 0xffffn))];
});

create(122 as u8, "store_ind_u32", ({ wA, wB, vX }) => {
  const location = toSafeMemoryAddress(wB + vX);
...
create(123 as u8, "store_ind_u64", ({ wA, wB, vX }) => {
  const location = toSafeMemoryAddress(wB + vX);
  return [IxMod.memory(location, encodeWithCodec(E_8, wA))];
});

// # load unsigned
create(124 as u8, "load_ind_u8", ({ rA, wB, vX }, context) => {
  const location = toSafeMemoryAddress(wB + vX);
  // TODO: memory fault?
  return [
...
create(126 as u8, "load_ind_u16", ({ rA, wB, vX }, context) => {
  const location = toSafeMemoryAddress(wB + vX);
...
create(128 as u8, "load_ind_u32", ({ rA, wB, vX }, context) => {
  const location = toSafeMemoryAddress(wB + vX);
...
create(130 as u8, "load_ind_u64", ({ rA, wB, vX }, context) => {
  const location = toSafeMemoryAddress(wB + vX);
...
// # load signed
create(125 as u8, "load_ind_i8", ({ rA, wB, vX }, context) => {
  const location = toSafeMemoryAddress(wB + vX);
...
create(127 as u8, "load_ind_i16", ({ rA, wB, vX }, context) => {
  const location = toSafeMemoryAddress(wB + vX);
...
create(129 as u8, "load_ind_i32", ({ rA, wB, vX }, context) => {
  const location = toSafeMemoryAddress(wB + vX);
...
// math
create(131 as u8, "add_imm_32", ({ rA, wB, vX }) => {
  return [IxMod.reg(rA, X_4((wB + vX) % 2n ** 32n))];
});

create(132 as u8, "and_imm", ({ rA, wB, vX }) => {
  return [IxMod.reg(rA, wB & BigInt(vX))];
});

create(133 as u8, "xor_imm", ({ rA, wB, vX }) => {
  return [IxMod.reg(rA, wB ^ BigInt(vX))];
});

create(134 as u8, "or_imm", ({ rA, wB, vX }) => {
  return [IxMod.reg(rA, wB | BigInt(vX))];
});

create(135 as u8, "mul_imm_32", ({ rA, wB, vX }) => {
  return [IxMod.reg(rA, (wB * BigInt(vX)) % 2n ** 32n)];
});

create(136 as u8, "set_lt_u_imm", ({ rA, wB, vX }) => {
  return [IxMod.reg(rA, wB < vX ? 1 : 0)];
});

create(137 as u8, "set_lt_s_imm", ({ rA, wB, vX }) => {
  return [IxMod.reg(rA, Z8(wB) < Z8(BigInt(vX)) ? 1 : 0)];
});

create(138 as u8, "shlo_l_imm_32", ({ rA, wB, vX }) => {
  return [IxMod.reg(rA, X_4((wB << vX % 32n) % 2n ** 32n))];
});

create(139 as u8, "shlo_r_imm_32", ({ rA, wB, vX }) => {
  const wb = Number(wB % 2n ** 32n);
  return [IxMod.reg(rA, X_4(BigInt(wb >>> Number(vX % 32n))))];
});

create(140 as u8, "shar_r_imm_32", ({ rA, wB, vX }) => {
  const wb = Number(wB % 2n ** 32n);
  return [IxMod.reg(rA, Z8_inv(BigInt(Z4(wb) >> Number(vX % 32n))))];
});

create(141 as u8, "neg_add_imm_32", ({ rA, wB, vX }) => {
...
create(142 as u8, "set_gt_u_imm", ({ rA, wB, vX }) => {
  return [IxMod.reg(rA, wB > vX ? 1 : 0)];
});

create(143 as u8, "set_gt_s_imm", ({ rA, wB, vX }) => {
  return [IxMod.reg(rA, Z8(wB) > Z8(BigInt(vX)) ? 1 : 0)];
});

create(144 as u8, "shlo_l_imm_alt_32", ({ rA, wB, vX }) => {
  return [IxMod.reg(rA, X_4((vX << wB % 32n) % 2n ** 32n))];
});

create(145 as u8, "shlo_r_imm_alt_32", ({ rA, wB, vX }) => {
  return [IxMod.reg(rA, (Number(vX) >>> Number(wB % 32n)) as u32)];
});

create(146 as u8, "shar_r_imm_alt_32", ({ rA, wB, vX }) => {
  return [IxMod.reg(rA, Z8_inv(BigInt(Z4(vX % 2n ** 32n)) >> wB % 32n))];
});

create(147 as u8, "cmov_iz_imm", ({ rA, wB, vX }) => {
  if (wB === 0n) {
...
create(148 as u8, "cmov_nz_imm", ({ rA, wB, vX }) => {
  if (wB !== 0n) {
    return [IxMod.reg(rA, vX)];
  }
  return [];
...
create(149 as u8, "add_imm_64", ({ rA, wB, vX }) => {
  return [IxMod.reg(rA, (wB + BigInt(vX)) % 2n ** 64n)];
});

create(150 as u8, "mul_imm_64", ({ rA, wB, vX }) => {
  return [IxMod.reg(rA, (wB * BigInt(vX)) % 2n ** 64n)];
});

create(151 as u8, "shlo_l_imm_64", ({ rA, wB, vX }) => {
  return [IxMod.reg(rA, X_8((wB << BigInt(vX % 64n)) % 2n ** 64n))];
});

create(152 as u8, "shlo_r_imm_64", ({ rA, wB, vX }) => {
  return [IxMod.reg(rA, X_8(wB / 2n ** (BigInt(vX) % 64n)))];
});

create(153 as u8, "shar_r_imm_64", ({ rA, wB, vX }) => {
  const z8b = Z8(wB);
...
create(154 as u8, "neg_add_imm_64", ({ rA, wB, vX }) => {
  return [IxMod.reg(rA, (BigInt(vX) + 2n ** 64n - wB) % 2n ** 64n)];
});

create(155 as u8, "shlo_l_imm_alt_64", ({ rA, wB, vX }) => {
  return [IxMod.reg(rA, (BigInt(vX) << wB % 64n) % 2n ** 64n)];
});

create(156 as u8, "shlo_r_imm_alt_64", ({ rA, wB, vX }) => {
  return [IxMod.reg(rA, (BigInt(vX) / 2n ** (wB % 64n)) % 2n ** 64n)];
});

create(157 as u8, "shar_r_imm_alt_64", ({ rA, wB, vX }) => {
  return [IxMod.reg(rA, Z8_inv(Z8(BigInt(vX)) >> wB % 64n))];
});

create(158 as u8, "rot_r_64_imm", ({ rA, wB, vX }) => {
  const shift = vX % 64n;
...
create(159 as u8, "rot_r_64_imm_alt", ({ rA, wB, vX }) => {
  const shift = wB % 64n;
  const mask = 2n ** 64n - 1n;
  const value = vX;
  const result = (value >> shift) | ((value << (64n - shift)) & mask);
...
create(160 as u8, "rot_r_32_imm", ({ rA, wB, vX }) => {
  const shift = vX % 32n;
...
create(161 as u8, "rot_r_32_imm_alt", ({ rA, wB, vX }) => {
  const shift = wB % 32n;
  const mask = 2n ** 32n - 1n;
