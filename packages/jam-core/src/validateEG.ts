...
/**
 * $(0.5.3 - 11.25) | calculates R in it
 */
...
export const assertEGValid = (
  ...
  for (const { workReport } of extrinsic) {
    // $(0.5.3 - 11.3) | Check the number of dependencies in the workreports
    if (
    ...
    // $(0.5.3 - 11.8) | check work report total size
  ...
  // $(0.5.3 - 11.24) - make sure they're ordered and uniqueby coreindex
  ...
  for (const { credential } of extrinsic) {
    ...
    // $(0.5.3 - 11.25) | creds must be ordered by their val idx
  ...
  // $(0.5.3 - 11.42) | check the result serviceIndex & codeHash match what we have in delta
  for (const _w of w) {
  ...
  // $(0.5.3 - 11.30) | check gas requiremens
  for (const wr of w) {
  ...
  //  // $(0.5.3 - 11.30) | no reports can be placed in core when there is something pending
  ...
  for (const refinementContext of x) {
    ...
    // $(0.5.3 - 11.34) each lookup anchor block within `L` timeslot
    if (
